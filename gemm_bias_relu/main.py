...
def matmul_add(N, L, M, dtype):
    ...
    matmul_bias = te.compute((M, N), lambda i, j: matmul[i, j] + C[i, j], name="matmul_bias")
    out = te.compute((M, N), lambda i, j: tvm.tir.max(matmul_bias[i, j], 0.0), name="out")
...
out_tvm = tvm.nd.max(out_tvm.shape, device=dev)
